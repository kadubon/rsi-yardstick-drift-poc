...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Render a simple SVG plot for PoC v3 held-out results.")
...
def main() -> None:
    ...
    row_height = 72
    width = 1180
    height = 120 + len(rows) * row_height + 120
    panel_x = [left, left + 180, left + 360, left + 540]
    colors = ["#2b6cb0", "#c05621", "#2f855a", "#b83280"]
    titles = ["Strict score", "Proxy admission", "Proxy mean score", "Challenge failures"]

    svg_lines = [
        f'<svg xmlns="http://www.w3.org/2000/svg" width="{width}" height="{height}" viewBox="0 0 {width} {height}">',
        '<rect width="100%" height="100%" fill="#fffaf0" />',
        '<text x="24" y="34" font-size="22" font-family="Georgia, serif" fill="#1a202c">RSI yardstick drift PoC v3</text>',
    ]
    ...
    for row_index, row in enumerate(rows):
        ...
        admission_rate = float(row["proxy_admission_rate"])
        proxy_mean = float(row["proxy_mean_item_score"])
        ...
        values = [(strict_score, 1.0), (admission_rate, 1.0), (proxy_mean, 1.0), (challenge, max_challenge)]
        for index, (value, maximum) in enumerate(values):
    ...
    eval_flip_payload = next(
        json.loads(line) for line in flip_lines if '"record_type": "flip_split_summary"' in line and '"split": "evaluation"' in line
    )
    footer = (
        "Held-out ruler drift: fail->pass={fail_to_pass_count}, pass->fail={pass_to_fail_count}, "
        "net={net_admission_difference}, mean-score-delta={proxy_mean_item_score_delta}"
    ).format(**eval_flip_payload)
    svg_lines.append(
        f'<text x="24" y="{height - 36}" font-size="13" font-family="Verdana, sans-serif" fill="#2d3748">{footer}</text>'
    )
