...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Analyze same-output admission flips between two v3 proxy rulers.")
    ...
    parser.add_argument("--condition-label", default="ruler_drift_only_v3")
...
def main() -> None:
    ...
    split_stats = defaultdict(lambda: {"fail_to_pass": 0, "pass_to_fail": 0, "score_delta_sum": 0.0, "net_admission_delta": 0, "total": 0})
    ...
    for task_id in shared_task_ids:
        ...
        score_delta = round(float(drift["proxy_mean_item_score"]) - float(base["proxy_mean_item_score"]), 4)
        split_stats[split_name]["fail_to_pass"] += int(fail_to_pass)
        split_stats[split_name]["pass_to_fail"] += int(pass_to_fail)
        split_stats[split_name]["score_delta_sum"] += score_delta
        split_stats[split_name]["net_admission_delta"] += int(drift["proxy_pass"]) - int(base["proxy_pass"])
        ...
        task_records.append(
            {
                ...
                "baseline_proxy_mean_item_score": base["proxy_mean_item_score"],
                "drifted_proxy_mean_item_score": drift["proxy_mean_item_score"],
                "proxy_mean_item_score_delta": score_delta,
            }
        )
        ...
        records.append(
            {
                ...
                "net_admission_difference": int(stats["net_admission_delta"]),
                "proxy_mean_item_score_delta": round(float(stats["score_delta_sum"]) / total, 4) if total else 0.0,
                "total_tasks": total,
            }
        )
