...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run v3 delayed strict audit for a condition/proxy pair.")
...
def main() -> None:
    ...
    split_stats = {
        "calibration": {"strict": 0, "proxy": 0, "score_sum": 0.0, "confirmed": 0, "disagreement": 0, "challenge": 0, "total": 0, "valid_json": 0},
        "evaluation": {"strict": 0, "proxy": 0, "score_sum": 0.0, "confirmed": 0, "disagreement": 0, "challenge": 0, "total": 0, "valid_json": 0},
    }
    ...
    for task_id in shared_task_ids:
        ...
        category = str(proxy_by_task[task_id]["task_category"])
        category_stats.setdefault((split_name, category), {"strict": 0, "proxy": 0, "score_sum": 0.0, "confirmed": 0, "disagreement": 0, "challenge": 0, "total": 0, "valid_json": 0})
        split_stats[split_name]["strict"] += int(strict_pass)
        split_stats[split_name]["proxy"] += int(proxy_pass)
        split_stats[split_name]["score_sum"] += float(proxy_by_task[task_id]["proxy_mean_item_score"])
        ...
        split_stats[split_name]["valid_json"] += int(strict_by_task[task_id]["valid_json"])
        category_stats[(split_name, category)]["strict"] += int(strict_pass)
        category_stats[(split_name, category)]["proxy"] += int(proxy_pass)
        category_stats[(split_name, category)]["score_sum"] += float(proxy_by_task[task_id]["proxy_mean_item_score"])
        category_stats[(split_name, category)]["confirmed"] += int(strict_confirmed)
        category_stats[(split_name, category)]["disagreement"] += int(disagreement)
        category_stats[(split_name, category)]["challenge"] += int(challenge_failure)
        category_stats[(split_name, category)]["total"] += 1
        category_stats[(split_name, category)]["valid_json"] += int(strict_by_task[task_id]["valid_json"])
        task_records.append(
            {
                ...
                "proxy_mean_item_score": proxy_by_task[task_id]["proxy_mean_item_score"],
                "valid_json": strict_by_task[task_id]["valid_json"],
        ...
        records.append(
            {
                ...
                "proxy_admission_rate": round(proxy_admitted / total, 4) if total else 0.0,
                "proxy_mean_item_score": round(float(stats["score_sum"]) / total, 4) if total else 0.0,
                ...
                "valid_json_count": int(stats["valid_json"]),
    ...
    for (split_name, category), stats in sorted(category_stats.items()):
        ...
        records.append(
            {
                "record_type": "audit_category_summary",
                "audit_id": audit_id,
                "condition": condition,
                "proxy_profile": proxy_label,
                "split": split_name,
                ...
                "proxy_mean_item_score": round(float(stats["score_sum"]) / total, 4) if total else 0.0,
                "proxy_admitted_count": proxy_admitted,
                "strict_confirmed_count": int(stats["confirmed"]),
                "challenge_failure_count": int(stats["challenge"]),
                "disagreement_count": int(stats["disagreement"]),
                "valid_json_count": int(stats["valid_json"]),
                "audited_precision": round(int(stats["confirmed"]) / proxy_admitted, 4) if proxy_admitted else None,
                "total_tasks": total,
            }
        )
